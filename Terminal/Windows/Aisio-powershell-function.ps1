...
function DSA_Export_Keys_PEM {
    .\aisio -dsa -export -key -param "$BASE" -n "$N" -e "$E" -d "$D" -p "$P" -q "$Q" -dp "$DP" -dq "$DQ" -qi "$QI" -out -pem
}

function DSA_Export_Keys_DER {
    .\aisio -dsa -export -key -param "$BASE" -n "$N" -e "$E" -d "$D" -p "$P" -q "$Q" -dp "$DP" -dq "$DQ" -qi "$QI" -out -der "$BASE"
}