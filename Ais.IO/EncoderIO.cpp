...
int Base32Encode(const char* input, char* output, int outputSize) {
    ...
    size_t inputLength = std::strlen(input); // 块
    size_t outputNeeded = ((inputLength + 4) / 5) * 8; // – 5 竊癸莱 8  Base32 才

    if (outputSize < static_cast<int>(outputNeeded + 1)) // 挡Ю '\0'
        return -2; // 块絯侥跋ぃì
    ...
    return static_cast<int>(outputIndex); // 龟悔ネΘ块
}
