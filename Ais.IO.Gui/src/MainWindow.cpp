...
void MainWindow::onRunBaseEncode() {
    ...
    QJsonObject obj{
        {"type",     mode},
        {"content",  input},      // Qt 會在 JSON 裡用 UTF-8
        {"encoding", "UTF8"}
    };
    ...
    worker_->runAisioJson("BaseEncode", QJsonDocument(obj).toJson(QJsonDocument::Compact));
}
...
void MainWindow::onRunBaseDecode() {
    ...
    connect(worker_, &CryptoWorker::finished, this, &MainWindow::onWorkerFinished);
    //worker_->runAisio(args);
    worker_->runAisioJson("BaseDecode", QJsonDocument(obj).toJson(QJsonDocument::Compact));
}
